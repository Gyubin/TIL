...
- Database scale out
    ...
    + 그래도 안되면 Sharding이란 방법을 쓴다. 즉 마스터를 2개로 쪼개서 1-1000번 유저는 Master1을, 1000-2000번 유저는 Master2를 쓰는 방식.
...
- 마지막 고급 세팅: Public Accessible은 같은 VPC가 아니더라도 접속하게 할 것인지를 선택하는 것. db 이름 설정해준다. 백업은 어느 기간만큼 백업을 할 것인지 설정하는 것인데 7일이면 가장 최근 7일까지는 계속 아마존에서 백업을 해놓는다는 의미다. 백업본을 restore할 때는 그 날짜의 DB 새로운 인스턴스를 생성해준다. 길면 비싸짐.
- 백업을 직접 해줄 수도 있는데 그것이 Snapshot이다. 스냅샷을 찍는 것은 백업한다는 의미고, 역시 자동 백업처럼 스냅샷을 restore하는 것은 새로운 인스턴스를 생성하게 된다.

### 7.3 Multi AZ

![multi-az](http://i.imgur.com/L5j44TU.png)

- 기존에 만들어진 DB 인스턴스를 변경할 수도 있다. Multi-AZ Deployment 부분을 Yes 한다. 변경사항이 바로 적용될지, 예정된 시각에 적용될지는 아래 Maintenance 부분에서 체크해주면 된다.
- DB가 여러 개 있어서 성능을 향상시킨다는게 아니고, 스탠바이 상태로 예비 데이터 베이스가 있는 것이다. 문제가 발생했을 때 바로 전환되도록.
- 또한 DB에 변경을 할 때도 서비스를 계속 이어갈 수 있다. 예비 DB가 동작할 것이기 때문. 짧게 멈추더라도 multi-az를 하지 않은 것과는 엄청난 차이가 있음.

### 7.4 Scale out

- DB에서는 읽기(read)와 쓰기(insert, update, delete)로 크게 작업이 나뉜다. 그래서 스케일 아웃은 읽기를 전담하는 slave instance들을 추가로 만드는 것으로 진행된다.
...
- terminate할 때는 Beanstalk를 지우면 만들어진 모든게 지워지므로 편리하지만 위험할 수도 있다. 다만 지울 때 S3는 바로 지워지지 않는데 property 탭의 permission 메뉴에서 bucket policy를 아예 지워주면 된다. 그 후에 delete하면 바로 지워진다.
