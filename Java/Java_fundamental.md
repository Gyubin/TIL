...
- static variable
    + 변수의 값이 항상 일정하다면 static으로 쓴다.
    + 자료형 앞에 붙여주면 된다.
    + 객체마다 값이 생기는 것이 아니라 클래스에 하나만 존재한다. 모든 객체가 이 변수를 공유하는 것이 된다. 메모리 관리에 좋다.
- static method
    + 객체를 만들지 않고도 클래스에서 바로 호출할 수 있는 메소드다.
    + 역시 리턴값을 나타내는 자료형 정의 앞에 static을 붙여준다.
    + static method에서 instance variable로 접근은 불가. static variable에 한해 접근해서 값을 바꿀 수 있다.
    + 주로 유틸성 메소드가 static으로 만들어진다. 오늘 날짜, 숫자에 콤마 추가하기 등.
